...
function SavedCard({ ad }: { ad: SavedAd }) {
  return (
    <article className="rounded-[26px] bg-white p-4">
      <div className="h-[280px] w-full overflow-hidden rounded-[18px] bg-white">
        <img
          src={ad.image}
          alt={ad.title}
      ...
      <div className="pt-4">
        <div className="mb-2 flex items-center justify-between gap-2">
          <h3 className="text-[42px] font-semibold leading-none text-ink">{ad.price}</h3>
          <span className="rounded-[12px] bg-[#f5ebdc] px-3 py-1.5 text-[16px] text-[#ff9715]">New</span>
        </div>
        <h4 className="mb-2 text-[20px] font-medium leading-tight text-ink">{ad.title}</h4>
        <p className="mb-2 text-[16px] leading-[1.4] text-[#6d6a74]">{ad.description}</p>
        <small className="flex items-center gap-1.5 text-[16px] text-[#4b4a54]">
          <LocationPin />
          {ad.location}
        </small>
      </div>
  ...
  return (
    <div className="min-h-screen bg-page text-ink">
      ...
      <main className="mx-auto w-full max-w-[1728px] px-4 pb-16 pt-8 sm:px-6 lg:px-12 lg:pb-24">
        ...
        <section className="grid max-w-[980px] grid-cols-1 gap-6 md:grid-cols-2">
          {savedAds.map((ad) => (
