...
## Phase 2 [ ] Admin Layout/Nav Cleanup

### Goal
Make admin navigation accurate and implementation-aligned.

### Scope
- Remove dead admin nav links (Settings, Analytics) until implemented.
- Unify desktop/mobile admin nav from one source of truth.
- Ensure route list exactly matches implemented admin pages.

### Files likely touched
- qwik_frontend/src/components/admin/AdminSidebar.tsx
- qwik_frontend/src/components/admin/AdminLayout.tsx
- qwik_frontend/src/App.tsx
- qwik_frontend/src/constants/routes.ts

### Backend work
- None required.

### Frontend work
- Remove unimplemented nav entries.
- Keep nav route parity between sidebar and mobile chips.

### Validation checklist
- [ ] No admin nav item points to missing route/page.
- [ ] Desktop and mobile admin nav lists match.
- [ ] Route constants reflect implemented pages only.

### Risk notes
- If future phases start quickly, nav changes may need near-term re-expansion.

### Completion criteria
- Admin nav contains only working destinations.
